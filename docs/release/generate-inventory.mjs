...
function writeJson(fileName, value) {
  writeFileSync(path.join(releaseDirectory, fileName), `${JSON.stringify(stable(value), null, 2)}\n`, "utf8");
}
...
function sourceProperties(lockfile, lockHash) {
  return [
    ...
    { name: "tactile:generated-by", value: "docs/release/generate-inventory.mjs" },
  ];
}
...
function inventoryMarkdown(inventory) {
  ...
  return [
    ...
    "",
    "<!-- Generated by docs/release/generate-inventory.mjs. Do not edit this file directly. -->",
    "",
    ...
    "```",
    "",
    "The command reads package-lock.json and src-tauri/Cargo.lock, runs the pinned package-manager SBOM command and locked Cargo metadata query, and writes only the four generated artifacts in docs/release/.",
    "",
  ].join("\n");
}
...
writeFileSync(path.join(releaseDirectory, "third-party-inventory.md"), inventoryMarkdown(inventory), "utf8");
